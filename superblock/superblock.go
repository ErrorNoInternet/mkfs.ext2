...
func New(
	...
	if superblock.NumBlockGroups > 1 {
		copyBlockGroupIds = append(copyBlockGroupIds, 1)
		last3 := 3
		for last3 < superblock.NumBlockGroups {
			copyBlockGroupIds = append(copyBlockGroupIds, last3)
			last3 *= 3
		}
		last5 := 5
		for last5 < superblock.NumBlockGroups {
			copyBlockGroupIds = append(copyBlockGroupIds, last5)
			last5 *= 5
		}
		last7 := 7
		for last7 < superblock.NumBlockGroups {
			copyBlockGroupIds = append(copyBlockGroupIds, last7)
			last7 *= 7
		}
	}
	...
	superblock.NumFreeBlocks = (superblock.NumBlocks - superblock.FirstBlockId - superblock.InodeTableBlocks*superblock.NumBlockGroups - 2*superblock.NumBlockGroups - (1+superblock.BgdtBlocks)*(len(copyBlockGroupIds)+1))

	superblock.LastBgId = superblock.NumBlockGroups - 1
	overhead := 2 + superblock.InodeTableBlocks
	targetIndex := -1
	for index, item := range copyBlockGroupIds {
	...
	if overhead > superblock.NumBlocks-(superblock.LastBgId*superblock.NumBlocksPerGroup+superblock.FirstBlockId) {
		if targetIndex != -1 {
			copyBlockGroupIds = append(
				copyBlockGroupIds[:targetIndex],
				copyBlockGroupIds[targetIndex+1:]...,
			)
		}
	...
	superblock.LogBlockSize = superblock.BlockSize >> 1
